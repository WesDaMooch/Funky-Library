...
void MixMatchApp::runFrame2()
{
    ...
    if (ImGui::Begin("Funky Libaray", &open, windowFlags))
    {
        ...
        if (ImGui::BeginMenuBar())
        {
            ...
            {
                if (ImGui::MenuItem("Library manager"))
                {
                    managerPopup = true;
                }
        ...
        // Left panel
        {
            ImGui::BeginChild(
                "Left Panel", 
                ImVec2(ImGui::GetContentRegionAvail().x * 0.25f, ImGui::GetContentRegionAvail().y), 
                ImGuiChildFlags_ResizeX,
                ImGuiWindowFlags_None
            );

            // Add track and search
            {
                ImGui::BeginChild(
                    "Left Top Panel", 
                ...
                ImGui::SetItemTooltip("Add track");
            ...
            // Release and label
            {
                ...
                std::vector<const Track*> tracksToDisplay;

                if (ImGui::BeginTabBar("Release Label Artist Tab"))
                {
                ...
                ImGui::BeginChild("Release Or Label Track List", ImVec2(0, 0));

                if (!tracksToDisplay.empty())
                {
        ...
        ImGui::SameLine();
        {
            ...
            auto* activeTrack = library.findTrackById(activeTrackId);

            if (activeTrack != nullptr)
            {
                const auto* artist = library.findArtistById(activeTrack->artistId);
                ImGui::Text(TextUtil::FormartTitle(artist->name, activeTrack->title).c_str());

                ImGui::SameLine();

                if (ImGui::Button(Text::ICON_EDIT))
                {
                    editTrackData.parseTrack(activeTrack, library);
                    editTrackPopup = true;
                    ImGui::OpenPopup("Edit Track");
                }

                drawInputTrackDataPopup(editTrackData, TrackInputMode::Edit);

                drawMixes(activeTrack);
        ...
        // Add track popup
        drawInputTrackDataPopup(addTrackData, TrackInputMode::Add);
...
void MixMatchApp::drawMixes(const Track* thisTrack)
{
    ...
    // Display mix table
    for (const auto& m : thisTrack->mix)
    {
        const auto* otherTrack = library.findTrackById(m.otherTrackId);

        if (otherTrack == nullptr)
            continue;

        const auto* otherArtist = library.findArtistById(otherTrack->artistId);

        if (otherArtist == nullptr)
            continue;

        ImGui::PushID(otherTrack->id);
        if (ImGui::Selectable(TextUtil::FormartTitle(otherArtist->name, otherTrack->title).c_str()))
            activeTrackId = otherTrack->id;
...
void MixMatchApp::drawInputTrackDataPopup(TrackInputData& data, TrackInputMode mode)
{
    bool& popupOpen = (mode == TrackInputMode::Add) ? addTrackPopup : editTrackPopup;
    ...
    const char* popupName = (mode == TrackInputMode::Add) ? "Add Track" : "Edit Track";

    const float fieldWidth = 500.0f;
    const float fieldHeight = ImGui::GetFrameHeight();

    if (ImGui::BeginPopupModal(popupName, &popupOpen, ImGuiWindowFlags_AlwaysAutoResize))
    {
        if (ImGui::BeginTabBar("Track Input Tab Bar"))
        {
            ...
            if (ImGui::BeginTabItem("Track", nullptr, flags))
            {
                // Artist input
                ...
                std::string artistStatusIcon;
                std::string artistTooltip;

                if (artistBufferEmpty)
                {
                    artistStatusIcon = Text::ICON_ERROR;
                    artistTooltip = "Requrires artist";
                }
                else if (artistInLibrary)
                {
                    artistStatusIcon = Text::ICON_CHECK;
                    artistTooltip = "Existing artist";
                }
                else
                {
                    artistStatusIcon = Text::ICON_ADD;
                    artistTooltip = "New artist";
                }

                ImGui::Text(artistStatusIcon.c_str());

                if (!artistTooltip.empty())
                    ImGui::SetItemTooltip(artistTooltip.c_str());
                ...
                if (!artistBufferEmpty && !trackBufferEmpty && artistInLibrary)
                {
                    ...
                    {
                        // Do not inclue self as duplicate if editing
                        if (mode == TrackInputMode::Edit && track.id == data.trackId)
                            continue;

                        const std::string existingTitle = TextUtil::ToLower(track.title);

                        if (trackName == existingTitle && artist->id == track.artistId)
                        {
                            trackDuplicate = true;
                            break;
                        }
                ...
                ImGui::Text(data.trackValid ? Text::ICON_ADD : Text::ICON_ERROR);

                std::string trackTooltip;

                if (trackBufferEmpty)
                {
                    trackTooltip = "Requires title";
                }
                else if (trackDuplicate)
                {
                    trackTooltip = "Duplicate track";
                }
                else
                {
                    trackTooltip = "New track";
                }

                if (!trackTooltip.empty())
                    ImGui::SetItemTooltip(trackTooltip.c_str());

                // Update save tooltip
                if (artistBufferEmpty)
                {
                    trackInputSaveTooltip = artistTooltip;
                }
                else if (trackBufferEmpty || trackDuplicate)
                {
                    trackInputSaveTooltip = trackTooltip;
                }
                ...
                {
                    ImGui::ColorPicker3("##Colour Picker",
                        (float*)&data.colour,
            ...
            {
                // Label input
                ImGui::InputText("Label", data.labelBuffer, sizeof(data.labelBuffer));

                const auto* label = library.findLabelByName(data.labelBuffer);
                bool labelBufferEmpty = (data.labelBuffer[0] == '\0');
                bool labelInLibrary = (label != nullptr);

                data.labelId = labelInLibrary ? label->id : -1;

                ImGui::SameLine();

                std::string icon = labelInLibrary ? Text::ICON_CHECK : Text::ICON_ADD;
                ...
                {
                    ImGui::BeginDisabled(labelBufferEmpty);
                    if (ImGui::SmallButton((icon + "##Label Status Button").c_str()))
                    {
                        if (!labelInLibrary && !labelBufferEmpty)
                            library.addLabel(data.labelBuffer);
                    }
                    ImGui::EndDisabled();
                }
                ...
                // Position input
                ImGui::InputText("Position", data.positionBuffer, sizeof(data.positionBuffer));
        ...
        // Save
        ImGui::BeginDisabled(!data.trackValid);
        if (ImGui::Button((mode == TrackInputMode::Add) ? "Add" : "Save"))
        {
            ...
            {
                // Create new label
                data.artistId = library.addArtist(data.artistBuffer);
                artistValid = (data.artistId >= 0);
            }
            ...
            {
                // Create new label
                data.labelId = library.addLabel(data.labelBuffer);
                labelValid = (data.labelId >= 0);
            }
            ...
            else if (data.releaseBuffer[0] == '\0')
            {
                // No label
                releaseValid = true;
            }
            ...
            {
                // Create new label
                data.releaseId = library.addRelease(data.releaseBuffer);
                releaseValid = (data.releaseId >= 0);
            }

            if (artistValid && labelValid && releaseValid && data.trackValid)
            {
                Track t = data.createTrack(mode);
          
                switch (mode) 
                {
                case TrackInputMode::Add:
                    library.addTrack(t);
                    break;
                
                case TrackInputMode::Edit:
                    library.editTrack(t);
                    break;
                }
        ...
        if (!trackInputSaveTooltip.empty())
            ImGui::SetItemTooltip(trackInputSaveTooltip.c_str());

        ImGui::SameLine();

        if (ImGui::Button((mode == TrackInputMode::Add) ? "Discard" : "Exit"))
        {
            data.clear();
            ImGui::CloseCurrentPopup();
        }
    ...
    if (ImGui::BeginPopupModal("Library Manager", &managerPopup))
    {
        if (ImGui::BeginTabBar("Manager Tab Bar"))
        {
            ...
            if (ImGui::BeginTabItem("Releases"))
            {
                ...
                {
                    bool selected = (releaseManagerData.selectedId == release.id);
                    bool& checked = releaseManagerData.checked[release.id];

                    ImGui::PushID(release.id);
                    ImGui::AlignTextToFramePadding();
                    if (ImGui::Selectable("##selectable", selected, ImGuiSelectableFlags_AllowOverlap))
                        releaseManagerData.selectedId = release.id;
                    ImGui::SameLine(0, 0);
                    ImGui::Checkbox("##check", &checked);
                    ImGui::SameLine();
                    ImGui::Text(release.name.c_str());
                ...
                // Right panel - release details
                ImGui::SameLine();

                ImGui::BeginChild("Release Details", ImVec2(rightPanelWidth, 0));
                    ...
                    if (release)
                    {
                        ...
                        for (auto& [releaseId, check] : releaseManagerData.checked)
                        {
                            if (check)
                            {
                                ...
                                ImGui::Text(checkedRelease->name.c_str());
                            }
                        }
...
#pragma region MainSearchBar
    ...
    if (ImGui::Button(Text::ICON_ADD, mainSearchBarButtonSize))
    {
        addTrackData = {};
    ...
    // Map button 
    if (ImGui::Button(Text::ICON_HALFSTAR, mainSearchBarButtonSize))
    {
        showTrackMap = !showTrackMap;

        if (showTrackMap)
            rebakeTrackMap(library.getCatalogueForDisplay());
    }
    ...
    if (showMainLibary)
    {
        const std::vector<Track> mainLibrary = library.searchAndSort(mainSearchBuffer, mainSearchSort);
...
#pragma region TrackMap
    ...
    if (showTrackMap)
    {
        ...
        if (hoveredNodeId > -1)
        {
            const Track* t = library.getTrackForDisplay(hoveredNodeId);
...
#pragma region ActiveTrackHeader
        ...
        const Track* activeTrack = library.getTrackForDisplay(activeTrackId);
        ...
        ImVec2 editSelSize = ImGui::CalcTextSize(Text::ICON_EDIT);
...
#pragma region ActiveTrackMixes
        ...
        // Add new mix button
        if (ImGui::Button(showAddMixWindow ? Text::ICON_REMOVE : Text::ICON_ADD))
        ...
        if (showAddMixWindow)
        {
            ...
            const std::vector<Track> mixSearchLibrary =
                library.searchAndSort(mixSearchBuffer, Library::TrackSort::Artist);
            ...
            if (mixToAddId > -1)
            {
                library.addMix(mixToAddId, activeTrackId);
...
#pragma endregion ActiveTrackMixes
    ...
    case MainPage::LABEL:
    {
        const std::vector<const Track*> labelLibrary = library.getLabel(activeLabel);
    ...
    {
        const std::vector<const Track*> releaseLibrary = library.getRelease(activeRelease);
...
void MixMatchApp::deleteConformationPopup(DeleteData& data, DeleteConformationMode mode)
{
    ...
    if (ImGui::Button("Yes"))
    {
        ...
        case DeleteConformationMode::TRACK:
            library.removeTrack(data.trackId);
            break;
        case DeleteConformationMode::MIX:
            library.removeMix(data.trackId, data.mixId);
            break;
        }
...
void MixMatchApp::displayLabelTable(std::vector<const Track*> trackList)
{
    ...
    {
        float ratingSize = ImGui::CalcTextSize(Text::ICON_STAR).x * 5;
        ImGui::TableSetupColumn("Artist & Title", ImGuiTableColumnFlags_WidthFixed);
...
void MixMatchApp::displayMixTable(const std::vector<Mix>& mixList)
{
    ...
    static float iconWidth = ImGui::CalcTextSize(Text::ICON_SYNCALT).x;
    ...
    {
        const Track* t = library.getTrackForDisplay(m.id);
        ...
        ImGuiTableFlags_Hideable))
    {
        ...
        float ratingSize = ImGui::CalcTextSize(Text::ICON_STAR).x * 5;
        float removeSize = ImGui::CalcTextSize(padTableText(Text::ICON_REMOVE).c_str()).x;
        ...
        for (const MixTableItem& item : items)
        {
            ...
            if (ImGui::Selectable("##Direction", false))
            {
                ...
                library.editMix(editedMix, activeTrackId);
            }
            ImGui::PopStyleColor(3);
            ImGui::SameLine();
            ...
            if (newRating > -1)
            {
                Track editedTrack = *library.getTrack(item.id);
                editedTrack.rating = newRating;
                library.editTrack(editedTrack);
            }
            ...
            if (drawTableButton(Text::ICON_CLOSE))
            {
    ...
    if (ImGui::BeginPopupModal("Edit Mix Note", &showEditMixNotePopup, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        if (ImGui::InputTextMultiline("##Mix Note", buf, sizeof(buf)))
        {
            ...
            library.editMix(inputMixData, activeTrackId);
        }
    ...
    if (ImGui::BeginPopupModal("Delete Mix",
        ...
        ImGuiWindowFlags_AlwaysAutoResize))
    {
        const Track* track = library.getTrackForDisplay(deleteMixData.id);
        ...
        if (ImGui::Button("Yes"))
        {
            library.removeMix(activeTrackId, deleteMixData.id);
            ImGui::CloseCurrentPopup();
        }
